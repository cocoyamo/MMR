...
Let's revisit the `heroes` dataset as an example.
...
-   `geom_bar()` creates the bar chart.
    ...
    -   `stat = "summary"` calculates the mean value of the y-axis variable, which is `Strength` in this case.
...
1.  The graphs are separated by different intelligence levels.

2.  In the **low** intelligence group, there is only data for male superheroes' strength, indicating there are no female superheroes in the low intelligence group.

3.  The y-axis scales differ across the graphs. Although the program automatically adjusts the scale based on the data, this might hinder important information. For example, in the `good` and `high` intelligence groups, the difference is harder to discern. We will explain how to address this in the **scales** section.
...
From the plot, we can see that this may not be the best dataset to demonstrate a scatter plot, as the outliers are obviously affecting the overall aesthetics. Let's remove the male superheroes on the far right of the graph to get a clearer view. Since we are removing a row (superhero), we will use `filter()` to help with this.
...
With the outlier removed, the scatter plot is clearer. Let's examine the code:

In the `aes()` section, I've used both color **AND** shape to differentiate by `Gender`. Usually, one type of distinction is sufficient, but sometimes colors alone may be hard to differentiate. In such cases, adding another feature (e.g., shape) can improve readability.
...
I am a bit tired of using the same dataset, so let's switch to another new fun dataset.

This dataset is part of the `openintro` package and contains several datasets. We'll be using one called `movies`.
...
Let's demonstrate how to change colors in a graph with another dataset!

In this `lego_sample` dataset, there are various LEGO sets, pieces, recommended retail prices, prices on Amazon, sizes, and more.
